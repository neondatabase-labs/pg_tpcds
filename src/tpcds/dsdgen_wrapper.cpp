...
void validate_options(void) {
  ...
  msg[0] = '\0';
  if (is_set("PARALLEL")) {
    if (get_int("PARALLEL") < 2)
      strcat(msg, "PARALLEL must be >= 2\n");
    if (get_int("CHILD") < 1)
      strcat(msg, "CHILD must be >= 1\n");
  }
...
int main(int ac, char **av) {
  int i, tabid = -1, nCommandLineLength = 0, nArgLength;
  char *tname;
  ds_key_t kRowCount, kFirstRow, kRandomRow, kValidateCount;
  struct timeb t;
  tdef *pT;
  table_func_t *pF;
  ...
  /* if we are using shared memory to pass parameters to/from peers, then read
  ...
  **/
  ...
  for (i = (is_set("UPDATE")) ? S_BRAND : CALL_CENTER;
       (pT = getSimpleTdefsByNumber(i)); i++) {
    if (!pT->name)
      break;
    ...
    if (is_set("VALIDATE")) {
      ...
      for (; kValidateCount; kValidateCount--) {
        genrand_key(&kRandomRow, DIST_UNIFORM, 1, kRowCount, 0,
                    VALIDATE_STREAM);
