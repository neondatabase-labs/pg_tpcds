...
int set_option(const char *name, const char *param) {
  ...
  //*)o->name,
  //"Cannot process option"); 			set_flg(name); 		} else
  // clr_flg(name); 		res = 1; 		break; 	case OPT_INT:
  // if (o->action) { 			if ((res = o->action((char *)o->name, param))
  // <
  // 0) 				usage(NULL, "Bad parameter argument"); 			else
  // sprintf(parse_int, "%d",
  // res);
  //		}
