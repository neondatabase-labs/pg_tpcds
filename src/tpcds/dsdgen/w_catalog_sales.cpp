...
static void mk_master(void *info_arr, ds_key_t index) {
  ...
  r->cs_call_center_sk = (r->cs_sold_date_sk == -1) ? -1 : mk_join(CS_CALL_CENTER_SK, CALL_CENTER, r->cs_sold_date_sk);
...
static void mk_detail(void *info_arr, int bPrint) {
  ...
  auto *arr = (tpcds::TableLoader **)info_arr;
  ...
  if (nTemp < CR_RETURN_PCT) {
    ...
    auto &loader_catalog_returns = *(arr[1]);

    loader_catalog_returns.start()
        .addItemKey(rr->cr_returned_date_sk)
        .addItemKey(rr->cr_returned_time_sk)
        .addItem(rr->cr_item_sk)
        .addItemKey(rr->cr_refunded_customer_sk)
        .addItemKey(rr->cr_refunded_cdemo_sk)
        .addItemKey(rr->cr_refunded_hdemo_sk)
        .addItemKey(rr->cr_refunded_addr_sk)
        .addItemKey(rr->cr_returning_customer_sk)
        .addItemKey(rr->cr_returning_cdemo_sk)
        .addItemKey(rr->cr_returning_hdemo_sk)
        .addItemKey(rr->cr_returning_addr_sk)
        .addItemKey(rr->cr_call_center_sk)
        .addItemKey(rr->cr_catalog_page_sk)
        .addItemKey(rr->cr_ship_mode_sk)
        .addItemKey(rr->cr_warehouse_sk)
        .addItemKey(rr->cr_reason_sk)
        .addItem(rr->cr_order_number)
        .addItem(rr->cr_pricing.quantity)
        .addItemDecimal(rr->cr_pricing.net_paid)
        .addItemDecimal(rr->cr_pricing.ext_tax)
        .addItemDecimal(rr->cr_pricing.net_paid_inc_tax)
        .addItemDecimal(rr->cr_pricing.fee)
        .addItemDecimal(rr->cr_pricing.ext_ship_cost)
        .addItemDecimal(rr->cr_pricing.refunded_cash)
        .addItemDecimal(rr->cr_pricing.reversed_charge)
        .addItemDecimal(rr->cr_pricing.store_credit)
        .addItemDecimal(rr->cr_pricing.net_loss)
        .end();
  }

  auto &loader_catalog_sales = *(arr[0]);

  loader_catalog_sales.start()
      .addItemKey(r->cs_sold_date_sk)
      .addItemKey(r->cs_sold_time_sk)
      .addItemKey(r->cs_ship_date_sk)
      .addItemKey(r->cs_bill_customer_sk)
      .addItemKey(r->cs_bill_cdemo_sk)
      .addItemKey(r->cs_bill_hdemo_sk)
      .addItemKey(r->cs_bill_addr_sk)
      .addItemKey(r->cs_ship_customer_sk)
      .addItemKey(r->cs_ship_cdemo_sk)
      .addItemKey(r->cs_ship_hdemo_sk)
      .addItemKey(r->cs_ship_addr_sk)
      .addItemKey(r->cs_call_center_sk)
      .addItemKey(r->cs_catalog_page_sk)
      .addItemKey(r->cs_ship_mode_sk)
      .addItemKey(r->cs_warehouse_sk)
      .addItem(r->cs_sold_item_sk)
      .addItemKey(r->cs_promo_sk)
      .addItem(r->cs_order_number)
      .addItem(r->cs_pricing.quantity)
      .addItemDecimal(r->cs_pricing.wholesale_cost)
      .addItemDecimal(r->cs_pricing.list_price)
      .addItemDecimal(r->cs_pricing.sales_price)
      .addItemDecimal(r->cs_pricing.ext_discount_amt)
      .addItemDecimal(r->cs_pricing.ext_sales_price)
      .addItemDecimal(r->cs_pricing.ext_wholesale_cost)
      .addItemDecimal(r->cs_pricing.ext_list_price)
      .addItemDecimal(r->cs_pricing.ext_tax)
      .addItemDecimal(r->cs_pricing.coupon_amt)
      .addItemDecimal(r->cs_pricing.ext_ship_cost)
      .addItemDecimal(r->cs_pricing.net_paid)
      .addItemDecimal(r->cs_pricing.net_paid_inc_tax)
      .addItemDecimal(r->cs_pricing.net_paid_inc_ship)
      .addItemDecimal(r->cs_pricing.net_paid_inc_ship_tax)
      .addItemDecimal(r->cs_pricing.net_profit)
      .end();
