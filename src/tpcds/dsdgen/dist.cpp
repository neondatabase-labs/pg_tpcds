...
#include "dsdgen/dcomp.h"
#include "dsdgen/decimal.h"
#include "dsdgen/dist.h"
#include "dsdgen/error_msg.h"
#include "dsdgen/genrand.h"
#include "dsdgen/r_params.h"
...
d_idx_t *find_dist(const char *name) {
  ...
  if (!InitConstants::find_dist_init) {
    ...
    if (!InitConstants::find_dist_init) /* make sure no one beat us to it */
    {
      ...
      //			if ((ifp = fopen(get_str("DISTRIBUTIONS"),
      //"rb")) == NULL) { 				fprintf(stderr, "Error:
      // open of distributions failed:
      //"); 				perror(get_str("DISTRIBUTIONS"));
      // exit(1);
      //			}
      memcpy(&temp, read_ptr, sizeof(int32_t));
      read_ptr += sizeof(int32_t);
      //			if (fread(&temp, 1, sizeof(int32_t), ifp) !=
      // sizeof(int32_t)) { 				fprintf(stderr, "Error:
      // read of index count failed:
      //"); 				perror(get_str("DISTRIBUTIONS"));
      // exit(2);
      //			}
      entry_count = ntohl(temp);
      read_ptr = tpcds_idx + tpcds_idx_len - (entry_count * IDX_SIZE);
      //			if ((temp = fseek(ifp, -entry_count * IDX_SIZE,
      // SEEK_END)) < 0) { 				fprintf(stderr, "Error:
      // lseek to index failed: "); 				fprintf(stderr,
      // "attempting to reach
      ...
      for (i = 0; i < entry_count; i++) {
        ...
        //				if (fread(idx[i].name, 1, D_NAME_LEN,
        // ifp)
        //<
        // D_NAME_LEN) { 					fprintf(stderr,
        // "Error: read index failed (1): "); perror(get_str("DISTRIBUTIONS"));
        // exit(2);
        //				}
        ...
        memcpy(&temp, read_ptr, sizeof(int32_t));
        read_ptr += sizeof(int32_t);
        //				if (fread(&temp, 1, sizeof(int32_t),
        // ifp)
        //!=
        // sizeof(int32_t)) { 					fprintf(stderr,
        // "Error: read index failed (2): ");
        // perror(get_str("DISTRIBUTIONS"));
        // exit(2);
        //				}
...
static int load_dist(d_idx_t *di) {
  ...
  if (di->flags != FL_LOADED) /* make sure no one beat us to it */
  {
    ...
    for (i = 0; i < di->v_width; i++) {
      ...
      //			if (fread(&temp, 1, sizeof(int32_t), ifp) !=
      // sizeof(int32_t)) { 				fprintf(stderr, "Error:
      // read of type vector failed for '%s': ", di->name);
      // perror("load_dist()"); 				exit(3);
      //			}
    ...
    for (i = 0; i < di->v_width; i++) {
      *(d->value_sets + i) = (int *)malloc(di->length * sizeof(int32_t));
      ...
      for (j = 0; j < di->length; j++) {
        ...
        //!=
        // sizeof(int32_t)) { 					fprintf(stderr,
        // "Error: read of values failed: ");
        // perror("load_dist()"); 					exit(4);
        //				}
    ...
    if (di->name_space) {
      ...
      //			if (fread(d->names, 1, di->name_space *
      // sizeof(char), ifp) < 0) { 				fprintf(stderr,
      // "Error: read of names failed:
      //"); 				perror("load_dist()");
      // exit(599);
      //			}
    }
 ...
 */
int dist_op(void *dest, int op, const char *d_name, int vset, int wset,
            int stream) {
  d_idx_t *d;
  dist_t *dist;
...
int IntegrateDist(const char *szDistName, int nPct, int nStartIndex,
                  int nWeightSet) {
  d_idx_t *pDistIndex;
  int nGoal, nSize;
 ...
 * TODO: None
 */
int MatchDistWeight(void *dest, const char *szDist, int nWeight, int nWeightSet,
                    int ValueSet) {
  d_idx_t *d;
  dist_t *dist;
