...
void hierarchy_item(int h_level, ds_key_t *id, char **name, ds_key_t kIndex) {
  ...
  switch (h_level) {
    ...
    case I_BRAND:
      if (nLastClass == -1)
        ReportErrorNoLine(DBGEN_ERROR_HIERACHY_ORDER, "I_BRAND before I_CLASS",
                          1);
 ...
 * TODO: None
 */
void bitmap_to_dist(void *pDest, const char *distname, ds_key_t *modulus,
                    int vset, int stream) {
  int32_t m, s;
  char msg[80];
 ...
 * TODO: None
 */
void dist_to_bitmap(int *pDest, const char *szDistName, int nValue, int nWeight,
                    int nStream) {
 ...
 * TODO:
 */
void mk_word(char *dest, const char *syl_set, ds_key_t src, int char_cnt,
             int col) {
 ...
 *operations
 */
static char szXlate[16] = {'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H',
                           'I', 'J', 'K', 'L', 'M', 'N', 'O', 'P'};
 ...
 * TODO: None
 */
int embed_string(char *szDest, const char *szDist, int nValue, int nWeight,
                 int nStream) {
  int nPosition;
  char *szWord = NULL;

  pick_distribution(&szWord, szDist, nValue, nWeight, nStream);
  nPosition = genrand_integer(NULL, DIST_UNIFORM, 0,
                              strlen(szDest) - strlen(szWord) - 1, 0, nStream);
