...
static void mk_detail(void *info_arr, int bPrint) {
  ...
  if (nTemp < WR_RETURN_PCT) {
    ...
    auto &loader_web_returns = *(arr[1]);

    loader_web_returns.start()
        .addItemKey(rr->wr_returned_date_sk)
        .addItemKey(rr->wr_returned_time_sk)
        .addItemKey(rr->wr_item_sk)
        .addItemKey(rr->wr_refunded_customer_sk)
        .addItemKey(rr->wr_refunded_cdemo_sk)
        .addItemKey(rr->wr_refunded_hdemo_sk)
        .addItemKey(rr->wr_refunded_addr_sk)
        .addItemKey(rr->wr_returning_customer_sk)
        .addItemKey(rr->wr_returning_cdemo_sk)
        .addItemKey(rr->wr_returning_hdemo_sk)
        .addItemKey(rr->wr_returning_addr_sk)
        .addItemKey(rr->wr_web_page_sk)
        .addItemKey(rr->wr_reason_sk)
        .addItem(rr->wr_order_number)
        .addItem(rr->wr_pricing.quantity)
        .addItemDecimal(rr->wr_pricing.net_paid)
        .addItemDecimal(rr->wr_pricing.ext_tax)
        .addItemDecimal(rr->wr_pricing.net_paid_inc_tax)
        .addItemDecimal(rr->wr_pricing.fee)
        .addItemDecimal(rr->wr_pricing.ext_ship_cost)
        .addItemDecimal(rr->wr_pricing.refunded_cash)
        .addItemDecimal(rr->wr_pricing.reversed_charge)
        .addItemDecimal(rr->wr_pricing.store_credit)
        .addItemDecimal(rr->wr_pricing.net_loss)
        .end();
  }
  auto &loader_web_sales = *(arr[0]);

  loader_web_sales.start()
      .addItemKey(r->ws_sold_date_sk)
      .addItemKey(r->ws_sold_time_sk)
      .addItemKey(r->ws_ship_date_sk)
      .addItemKey(r->ws_item_sk)
      .addItemKey(r->ws_bill_customer_sk)
      .addItemKey(r->ws_bill_cdemo_sk)
      .addItemKey(r->ws_bill_hdemo_sk)
      .addItemKey(r->ws_bill_addr_sk)
      .addItemKey(r->ws_ship_customer_sk)
      .addItemKey(r->ws_ship_cdemo_sk)
      .addItemKey(r->ws_ship_hdemo_sk)
      .addItemKey(r->ws_ship_addr_sk)
      .addItemKey(r->ws_web_page_sk)
      .addItemKey(r->ws_web_site_sk)
      .addItemKey(r->ws_ship_mode_sk)
      .addItemKey(r->ws_warehouse_sk)
      .addItemKey(r->ws_promo_sk)
      .addItem(r->ws_order_number)
      .addItem(r->ws_pricing.quantity)
      .addItemDecimal(r->ws_pricing.wholesale_cost)
      .addItemDecimal(r->ws_pricing.list_price)
      .addItemDecimal(r->ws_pricing.sales_price)
      .addItemDecimal(r->ws_pricing.ext_discount_amt)
      .addItemDecimal(r->ws_pricing.ext_sales_price)
      .addItemDecimal(r->ws_pricing.ext_wholesale_cost)
      .addItemDecimal(r->ws_pricing.ext_list_price)
      .addItemDecimal(r->ws_pricing.ext_tax)
      .addItemDecimal(r->ws_pricing.coupon_amt)
      .addItemDecimal(r->ws_pricing.ext_ship_cost)
      .addItemDecimal(r->ws_pricing.net_paid)
      .addItemDecimal(r->ws_pricing.net_paid_inc_tax)
      .addItemDecimal(r->ws_pricing.net_paid_inc_ship)
      .addItemDecimal(r->ws_pricing.net_paid_inc_ship_tax)
      .addItemDecimal(r->ws_pricing.net_profit)
      .end();
