...
int mk_w_inventory(void *info_arr, ds_key_t index) {
  ...
  r->inv_date_sk = jDate + (nTemp * 7); /* r->is updated weekly */

  /*
   * the join between item and r->is tricky. The item_id selected above
  ...
  genrand_integer(&r->inv_quantity_on_hand, DIST_UNIFORM, INV_QUANTITY_MIN, INV_QUANTITY_MAX, 0, INV_QUANTITY_ON_HAND);

  auto &loader = *(tpcds::TableLoader *)info_arr;

  loader.start()
      .addItemKey(r->inv_date_sk)
      .addItemKey(r->inv_item_sk)
      .addItemKey(r->inv_warehouse_sk)
      .addItem(r->inv_quantity_on_hand)
      .end();
...
ds_key_t sc_w_inventory(int nScale) {
  ...
  nDays += 1;
  nDays += 6;
  nDays /= 7; /* each items r->is updated weekly */
  kRes *= nDays;
