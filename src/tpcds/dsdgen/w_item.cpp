...
int mk_w_item(void *info_arr, ds_key_t index) {
  ...
  genrand_key(&r->i_manager_id, DIST_UNIFORM, (ds_key_t)nMin, (ds_key_t)nMax, 0,
              I_MANAGER_ID);
  ...
  nIndex =
      pick_distribution(&szMinPrice, "i_current_price", 2, 1, I_CURRENT_PRICE);
  ...
  genrand_decimal(&dMarkdown, DIST_UNIFORM, &dMinMarkdown, &dMaxMarkdown, NULL,
                  I_WHOLESALE_COST);
