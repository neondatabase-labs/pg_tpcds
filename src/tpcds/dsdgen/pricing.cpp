...
void set_pricing(int nTabId, ds_pricing_t *pPricing) {
  ...
  switch (nTabId) {
    ...
    case S_WLIN_PRICING:
      ...
      decimal_t_op(&pPricing->ext_sales_price, OP_MULT, &pPricing->sales_price,
                   &dQuantity);
      ...
      if (nCouponUsage <= 20) /* 20% of sales employ a coupon */
        decimal_t_op(&pPricing->coupon_amt, OP_MULT, &pPricing->ext_sales_price,
                     &dCoupon);
      else
    ...
    case WR_PRICING:
      ...
      decimal_t_op(&pPricing->refunded_cash, OP_MULT, &pPricing->refunded_cash,
                   &pPricing->net_paid);
