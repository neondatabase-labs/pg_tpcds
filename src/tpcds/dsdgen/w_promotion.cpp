...
int mk_w_promotion(void *info_arr, ds_key_t index) {
  ...
  nTemp = index;
  r->p_start_date_id =
      start_date.julian + genrand_integer(NULL, DIST_UNIFORM, PROMO_START_MIN,
                                          PROMO_START_MAX, PROMO_START_MEAN,
                                          P_START_DATE_ID);
  r->p_end_date_id =
  ...
  gen_text(&r->p_channel_details[0], PROMO_DETAIL_LEN_MIN, PROMO_DETAIL_LEN_MAX,
           P_CHANNEL_DETAILS);
